...
def extrair_informacoes_texto(pdf):
    with pdfplumber.open(pdf) as pdf_file:
        informacoes = []
...
def extrair_informacoes_software(pdf):
    with pdfplumber.open(pdf) as pdf_file:
        informacoes = []
...
def extrair_informacoes_infoblox(pdf_file):
    informacoes = []
...
def extrair_informacoes_f5_1(pdf_file):
    informacoes = []
...
def extrair_informacoes_f5_2(pdf_file):
    informacoes = []
...
def extrair_informacoes_palo_alto(pdf_file):
    informacoes_item = []
...
def upload():
    ...
    if pdf_file is None:
        return "Nenhum arquivo PDF recebido."
    ...
    if manufacturer == 'Dell':
        resultados_tabela = [resultado for resultado in extrair_tabela(pdf_file)]
        df_tabela = pd.DataFrame(resultados_tabela)

        resultados_texto = extrair_informacoes_texto(pdf_file)
        df_texto = pd.DataFrame(resultados_texto)

        resultados_software = extrair_informacoes_software(pdf_file)
        ...
        resultados_infoblox = extrair_informacoes_infoblox(pdf_file)
        df = pd.DataFrame(resultados_infoblox)

    elif manufacturer == 'F5 - 1':
        resultados_f5_1 = extrair_informacoes_f5_1(pdf_file) 
        df = pd.DataFrame(resultados_f5_1)
    
    elif manufacturer == 'F5 - 2':
        resultados_f5_2 = extrair_informacoes_f5_2(pdf_file)
        df = pd.DataFrame(resultados_f5_2)
    
    elif manufacturer == 'Palo Alto - Ingram':
        resultado_palo_alto = extrair_informacoes_palo_alto(pdf_file)
